...
func Pagesbar(url string, keyword string, results_max int64, pages int64, page int64, beginNum int64, endNum int64, style int64) (output template.HTML) {
	var raw string
	switch {
...
func RsaAesSendPacket(encrypt bool, status string, actionurl string, content string, aesKey string, aesPublicKey string, rsaPublicKey []byte) (*http.Response, error) {
	...
	*/
	...
	if encrypt {
		// AES对内容进行加密
		if aes_encrypt_content, err := AesCFBEncrypt(content, aesKey, aesPublicKey); err != nil {
			...
			if aseKey != "" {
				...
				if x, err := hex.DecodeString(aseKey); err == nil {
					...
					if rsa_decrypt_content, err := RsaDecrypt(x, rsaPrivateKey); err != nil {
						...
						//对AES数据进行AES解密得出内容
						if aes_decrypt_content, err := AesCFBDecrypt(string(content), aseKey, aesPublicKey); err != nil {
							return nil, err
						} else {
...
//发送报文
func SendPacket(status string, actionurl string, content string) (*http.Response, error) {
...
func GetThumbnails(content string) (thumbnails string, thumbnailslarge string, thumbnailsmedium string, thumbnailssmall string, err error) {
	...
	*/
	...
	if original_file, e := GetBanner(content); e == nil {
		...
		output_size_Large := "100x100"
		output_size_Medium := "48x48"
		output_size_Small := "32x32"
		output_align := "center"
		background := "#ffffff"
	...
	*/
	...
	if original_file := Url2local(localpath); original_file != "" {
		...
		if err := Thumbnail("thumbnail", input_file, original_file, "696x", output_align, background); err == nil {
			watermark_file := "./static/yougam/img/watermark.png"
...
func GetPage(url string) (string, error) {
	...
	ua := "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.1 (KHTML, like Gecko) Chrome/21.0.1180.92 Safari/537.1 Qunafei.Net Spider"
	client := &http.Client{}
...
func GetVideoAddress(input string) string {
	...
	text := string(rx.Find(b))
	if text != "" {
		text = strings.Replace(text, "'", "", -1)
		text = strings.Replace(text, "[", "", -1)
		text = strings.Replace(text, "]", "", -1)
	} else {
		cmd := exec.Command("you-get", "-F", "flv", "-u", input)
		b, _ := cmd.Output()
		cmd.Run()
